 ...
 * - change UintToAddressMap to HashToOfferStatusMap by renaming and adjusting methods
 * - add OfferStatus enum declaration
...
library EnumerableMap{
    ...
    */
    ...
    // HashToOfferStatusMap

    struct HashToOfferStatusMap {
        Map _inner;
    }
     ...
     * already present.
     */
    function set(HashToOfferStatusMap storage map, bytes32 key, OfferStatus value) internal returns (bool) {
        return _set(map._inner, key, bytes32(uint256(value)));
    }
     ...
     * Returns true if the key was removed from the map, that is if it was present.
     */
    function remove(HashToOfferStatusMap storage map, bytes32 key) internal returns (bool) {
     ...
     * @dev Returns true if the key is in the map. O(1).
     */
    function contains(HashToOfferStatusMap storage map, bytes32 key) internal view returns (bool) {
     ...
     */
    function length(HashToOfferStatusMap storage map) internal view returns (uint256) {
    ...
    * - `index` must be strictly less than {length}.
    */
    function at(HashToOfferStatusMap storage map, uint256 index) internal view returns (bytes32, OfferStatus) {
        (bytes32 key, bytes32 value) = _at(map._inner, index);
        return (key, OfferStatus(uint256(value)));
    }
     ...
     * - `key` must be in the map.
     */
    function get(HashToOfferStatusMap storage map, bytes32 key) internal view returns (OfferStatus) {
        return OfferStatus(uint256(_get(map._inner, key)));
    }

    /**
     * @dev Same as {get}, with a custom error message when `key` is not in the map.
     */
    function get(HashToOfferStatusMap storage map, bytes32 key, string memory errorMessage) internal view returns (OfferStatus) {
        return OfferStatus(uint256(_get(map._inner, key, errorMessage)));
    }
